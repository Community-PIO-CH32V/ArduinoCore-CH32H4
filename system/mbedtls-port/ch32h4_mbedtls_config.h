/* mbedTLS configuration for the CH32H41x.
 *
 * This is upstream's own mbedtls_config.h for 3.6.7, with a block of changes
 * appended at the end. Starting from upstream rather than from nothing is
 * deliberate: 3.6 routes hashes, ciphers and key types through PSA as well as
 * the legacy switches, and a config assembled by hand fails check_config.h
 * with a dozen "not all prerequisites" errors that say nothing about which
 * prerequisite. Editing a working config is tractable; building one is not.
 *
 * Every change is at the bottom, under CH32H4 OVERRIDES, so a future version
 * bump is a fresh copy of upstream's file plus that one block.
 *
 * Two of the changes are specific to this silicon:
 *
 *   MBEDTLS_AES_ALT               AES runs on the ECDC block; see aes_alt.c.
 *   MBEDTLS_ENTROPY_HARDWARE_ALT  entropy comes from the TRNG, through the
 *                                 whitening in ch32h4_rng.c -- the raw
 *                                 peripheral yields about ten bits per word.
 *
 * Version note: the 3.6 LTS line, not 4.x. mbedTLS 4 moved to PSA crypto
 * exclusively and dropped MBEDTLS_AES_ALT, which is the hook the ECDC
 * accelerator uses; getting the hardware back would mean writing a PSA driver
 * with no reference port to check it against.
 */
/**
 ...
 * CH32H4 OVERRIDES
 *
 * Everything above is upstream's file, unmodified.
 * ========================================================================== */

/* ---- this board --------------------------------------------------------- */

/* No file system, no sockets, no getrandom(). The TRNG is the only entropy
 * source, through ch32h4_mbedtls_port.c. */
#undef MBEDTLS_FS_IO
#undef MBEDTLS_NET_C
...
/* AES on the ECDC block. aes_alt.c/h replace the software implementation
 * wholesale, including the context type. */
#define MBEDTLS_AES_ALT

/* time() and gmtime_r() come from the core (ch32h4_rtc.c), so certificate
 * validity dates are actually checked. A board whose clock has not been set
 * rejects everything as "not yet valid" -- which is correct, and is why SNTP
 * is part of this core. Both are on by default upstream; named here because
 * they are load-bearing rather than incidental. */
#define MBEDTLS_HAVE_TIME
#define MBEDTLS_HAVE_TIME_DATE

/* mbedtls_ms_time() is supplied by ch32h4_mbedtls_port.c, from millis().
 * platform_util.c has implementations for POSIX and Windows and #errors on
 * anything else, so this is not optional on bare metal. It is deliberately
 * NOT derived from the RTC: it drives handshake and retransmission timeouts,
 * and must not jump by decades when SNTP sets the wall clock. */
#define MBEDTLS_PLATFORM_MS_TIME_ALT
...
/* The RSA and plain-DH key exchanges. Every one of them is either forbidden
 * in TLS 1.3 or forward-secrecy-free in 1.2, and no public server needs them
 * from a client that offers ECDHE. */
...
/* Curves. Keep the three the public web uses; each of the others is a few
 * kilobytes of tables for a curve no server will offer. */
...
/* Ciphers no TLS 1.2 or 1.3 ciphersuite in use needs. */
...
/* Record size.
 *
 * 16384 is the protocol maximum and costs about 32 KB of RAM for the two
 * buffers. 4096 in and 2048 out is the shape of an HTTPS GET: responses are
 * large, requests are not.
 *
 * A server insisting on a larger record fails the handshake with a clear
 * "buffer too small" rather than corrupting data. TLS 1.3 servers must honour
 * the record_size_limit extension, which mbedtls sends. */
#define MBEDTLS_SSL_IN_CONTENT_LEN   4096
#define MBEDTLS_SSL_OUT_CONTENT_LEN  2048

/* Kept, at about 8 KB of strings, and worth it: a TLS failure without it is a
 * negative hex number, and the difference between "the clock is wrong" and
 * "the certificate is not trusted" is exactly what someone debugging one
 * needs to know. */
#define MBEDTLS_ERROR_C
