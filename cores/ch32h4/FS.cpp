...
/* arduino-pico routes these through its own debug macro. There is no such
 * macro here, and adding a printf into the filesystem path would pull stdio
 * into every sketch that opens a file -- so the diagnostics are compiled out
 * by default and can be turned on with -DFS_DEBUG. */
#ifndef DEBUGV
#ifdef FS_DEBUG
