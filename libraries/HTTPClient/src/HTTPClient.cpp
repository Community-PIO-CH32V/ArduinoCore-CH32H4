    ...
    if (!_client()) {
        if (protocol == "https") {
               ...
               for sketches that call begin(url) without supplying their own. */
            {
                ...
                /* Nagle off: an HTTP request is a small write followed by a
                   wait for the reply, which is the exact pattern Nagle holds
                   back for up to 200 ms. */
                made->setNoDelay(true);
                _clientMade = made;
                /* The deleter, captured while the concrete type is still
                   known -- Client has no virtual destructor. */
                _deleteMade = [](Client *c) {
                    delete static_cast<EthernetClient *>(c);
                };
            }
            _clientGiven = false;
        }
...
/**
    ...
    @return success bool
*/
bool HTTPClient::begin(Client &client, const String& url) {
    // check for : (http: or https:)
    int index = url.indexOf(':');
    if (index < 0) {
        DEBUG_HTTPCLIENT("[HTTP-Client][begin] failed to parse protocol\n");
...
/**
    ...
    @return success bool
*/
bool HTTPClient::begin(Client &client, const String& host, uint16_t port, const String& uri, bool https) {
    // Disconnect when reusing HTTPClient to talk to a different host
    if ((_host != "") && (_host != host)) {
        _canReuse = false;
        disconnect(true);
...
Client& HTTPClient::getStream(void) {
    ...
    /* A concrete client, since Client itself is an interface. Returned by
       reference from getStream() when there is no connection, so that a
       caller which ignores the not-connected case gets a stream that reads
       nothing rather than a null dereference. */
    static EthernetClient empty;
    return empty;
}
...
bool HTTPClient::connect(void) {
    ...
    /* No setNoDelay() here. It is not on the Client interface -- it was on
       arduino-pico's WiFiClient -- and the client may be one the caller
       supplied and configured. The one this class creates for itself is
       constructed with Nagle already off, which is where that call's effect
       has gone; see _clientMade above. */
    return connected();
}
...
int HTTPClient::handleHeaderResponse() {
    ...
    // Hook up the HTTPStream to this Client already connected and headers decoded
