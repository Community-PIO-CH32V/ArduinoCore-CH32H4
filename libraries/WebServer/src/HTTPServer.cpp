...
/* The digest-authentication nonce comes from the hardware TRNG, not from
   random(). A predictable nonce lets an observer replay a captured
   Authorization header, which is the one thing the nonce exists to stop. */
extern "C" {
...
String HTTPServer::_getRandomHexString() {
    ...
    int i;
    for (i = 0; i < 4; i++) {
        sprintf(buffer + (i * 8), "%08lx", (unsigned long)ch32h4_rng_u32());
    }
