...
public:
    ...
    using HookFunction = std::function<ClientFuture(const String& method, const String& url, Client* client, ContentTypeFunction contentType)>;
    void addHook(HookFunction hook) {
        if (_hook) {
            auto previousHook = _hook;
            _hook = [previousHook, hook](const String & method, const String & url, Client * client, ContentTypeFunction contentType) {
...
protected:
    /* Cast rather than an overload: Print has write(const char*) but Client's
       write(const uint8_t*, size_t) is the one that takes a length, and a
       response body is bytes -- it may legitimately contain a zero. */
