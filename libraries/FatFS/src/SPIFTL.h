/*
    ...
    than a constant: this part erases 8192 bytes at a time, and 4096 on the
    480 KB variant, where the RP2040 this was written for is always 4096.
*/
...
#include <bitset>
#include <string.h>
/* bzero(), which this uses throughout. It comes in through string.h on the
 * RP2040's newlib and does not here. */
#include <strings.h>
#include <cassert>
#include <vector>
#include <list>
...
private:
    ...
    void openMetadataStreamForWrite() {
        ...
        for (int j = 0; j < metaEBs; j++) {
            ...
            const uint8_t *eb = _fi->readEB(i);
            metadataCRC.reset();
            metadataCRC.add(eb, ebBytes - 4);
            uint32_t crc = metadataCRC.get();
            bool err = memcmp(&crc, eb + ebBytes - 4, 4);
    ...
    inline void writeMetadata8b(uint8_t b, char *wb) {
        if (metadataEBoffset == ebBytes - 4) {
            uint32_t crc = metadataCRC.get();
            memcpy(&wb[flashWriteBufferSize - 4], &crc, 4);
            _fi->program(metadataEBList.front(), ebBytes - flashWriteBufferSize, wb, flashWriteBufferSize);
        ...
        for (int i = 0; (i < flashLBAs) && (curIdx < 8); i++) {
            if ((l2p_eb(i) == srcEB) && l2p_val(i)) {
                ...
                for (int j = 0; j < lbaBytes; j += sizeof(buff)) {
                    memcpy(buff, readAddr + lbaBytes * l2p_idx(i) + j, sizeof(buff));
                    _fi->program(destEB, lbaBytes * curIdx + j, buff, sizeof(buff));
                }
                clearLBAValid(srcEB);
